...
public class Graph {
   ...
   public void kruskal()
   {
       ...
       for(int i=nEdge;i>0;i--)
       {
           ...
           if(set.findSet(stnode)!=set.findSet(endnode)){
               ...
               edgeList[count++]=temp[i];
               
           }
       }
       printKruskalEdge(count);
   }
   public void printKruskalEdge(int count)
   {
       for(int i=0;i<count;i++)
       {
           System.out.println("stnode-"+edgeList[i].stNode+"-endnode-"+edgeList[i].endNode+"-weight-"+edgeList[i].edWeight);
       }
   }
  ...
  public void dijKstra()
  {
      ...
      myHeap.buildVMinHeap(); // build min heap using vertex
      
      while(myHeap.isEmpty())
      {
          Vertex u=myHeap.heapVExtractMin(); // extract vertex containing min dist
          
          for(int i=0;i<=list[mymap.get(u.label)].gettop();i++)
          {
               int v=list[mymap.get(u.label)].get(i);
              
              Relax(mymap.get(u.label), v, adjMat[mymap.get(u.label)][v]);
          }
      }
      printDijkstra();
  }
